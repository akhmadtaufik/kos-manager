...
export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('error', async (error: any, { event }) => {
    ...
    }, `[${reqId || 'UNKNOWN'}] An error occurred during request processing`)

    if (!event || event.handled) return

    // ONLY intercept and format JSON for API routes
    if (event && event.path.startsWith('/api/')) {
      ...
      if (isValidationError) {
        const response = sendErrorResponse(event, 400, 'Validation failed', formattedIssues, reqId)
      ...
      if (error.statusCode && error.statusCode >= 400 && error.statusCode < 500) {
        ...
        const response = sendErrorResponse(event, error.statusCode, error.statusMessage || error.message || 'Error', details, reqId)
        setResponseHeader(event, 'Content-Type', 'application/json')
        await send(event, JSON.stringify(response))
        return
      }
      ...
      const response = sendErrorResponse(event, 500, 'An unexpected internal error occurred.', null, reqId)
      setResponseHeader(event, 'Content-Type', 'application/json')
      await send(event, JSON.stringify(response))
    }
  })
